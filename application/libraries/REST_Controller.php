...
abstract class REST_Controller extends CI_Controller {
    ...
    protected function _perform_library_auth($username = '', $password = NULL)
    {
       ...
       // echo $this->{$auth_library_class}->$auth_library_function($username, $password);
		return (!empty($this->{$auth_library_class}->$auth_library_function($username, $password))) ? true : false;
    }
    /**
    ...
    protected function _prepare_basic_auth()
    {
        ...
        elseif ($http_auth !== NULL)
        {
            ...
            if (strpos(strtolower($http_auth), 'basic') === 0)
            {
				...
				//var_dump(list($username, $password) = explode(':', base64_decode(substr($this->input->server('HTTP_AUTHORIZATION'), 6))));
            }
        }


		//echo($this->_check_login($username, $password));
        // Check if the user is logged into the system

		//var_dump($this->_check_login($username, $password));
